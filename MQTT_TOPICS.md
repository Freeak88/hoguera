# MQTT — Topología de Tópicos (Honguera-01 → Aletheia)
...
|--------|----------------|------------|
| `honguera/honguera-01/sensors/sht40` | `{"temp_c": 23.4, "humidity_pct": 68.2, "ts": 1713738900}` | 5s |
| `honguera/honguera-01/sensors/co2` | `{"co2_ppm": 520, "ts": 1713738900}` | 5s |
| `honguera/honguera-01/sensors/ds18b20` | `{"temp_c": 28.1, "ts": 1713738900}` | 10s (opcional) |
...
|--------|---------|--------|
| `honguera/honguera-01/control/relay/heater` | `{"state": "ON", "duty_cycle_pct": 80}` | Cable 700W (SSR) |
| `honguera/honguera-01/control/relay/mister` | `{"state": "ON", "duration_s": 10}` | Transductores ultrasónicos |
| `honguera/honguera-01/control/fan/injection` | `{"state": "ON", "speed_pct": 100}` | Cooler 12V niebla |
...
**In:**
- `mqtt in` → `honguera/honguera-01/sensors/#`
- Parse JSON → Timestamp → Function (InfluxDB Line Protocol)

**Out:**
- `influxdb out` → bucket `sensors`
- `debug` → log
- `mqtt out` → `honguera/honguera-01/control/relay/heater` (cuando ML decide)
